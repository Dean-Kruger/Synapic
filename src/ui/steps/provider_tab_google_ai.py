"""
Google AI provider configuration tab for Step 2.
"""
...
class GoogleAIProviderTab(ProviderTabBase):
    """Google AI provider configuration tab."""
    ...
    def _setup_api_key_section(self):
        """Set up the Google AI API key configuration section."""
        ...
        self.api_key_var = ctk.StringVar(value=self.session.engine.google_ai_api_key or "")
        ...
        # Image only checkbox
        self.image_only_var = ctk.BooleanVar(value=self.session.engine.google_ai_image_models_only)
        ctk.CTkCheckBox(
            api_key_frame,
    ...
    def _load_models(self):
        """Load models from Google AI API."""
    ...
    def _display_models(self, models):
        """Display Google AI models in the models list."""
    ...
    def _add_model_to_list(self, model):
        """Add a Google AI model to the models list with custom formatting."""
        model_id = model.get('id', '')
        provider = model.get('provider', 'Google')
        capability = model.get('capability', 'Multi-modal')
    ...
    def _save_provider_config(self, model_id):
        """Save Google AI provider-specific configuration."""
        key = self.api_key_var.get().strip()
        ...
        try:
            ...
            logger.error(f"Error saving Google AI config: {e}")
    ...
    def refresh(self):
        ...
        if self.api_key_var:
            self.api_key_var.set(getattr(self.session.engine, "google_ai_api_key", "") or "")
...
def create_google_ai_tab(parent, session, worker, persist_preference_callback, filter_image_models_func):
    """Factory function to create a Google AI provider tab."""
    return GoogleAIProviderTab(
